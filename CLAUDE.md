...
3. **Exercise** — Notes card (see below), "This Set" card (weight/reps via a scrollable **number ribbon**, see below, "Finish set" → logs + auto-starts the rest timer), History card, then YouTube embed (iframe) + written form cues. Back → Day.
   - **Ribbon** (`src/components/Ribbon.jsx`) replaced the +/− steppers for weight (weighted mode) and reps/seconds (weighted, reps-only, time): a horizontal scroll-snap tape marked in `step` increments, a fixed center caret shows the selected tick, flanking chevrons nudge one step, and the big readout above is tappable to type an exact value. Mouse gets first-class support (click-drag the tape, wheel scrolls it) alongside touch/trackpad. Cardio's minutes/miles fields keep the old typed-stepper (not a weight/reps concept).
