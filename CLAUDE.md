...
- Day-button photos: **public/days/{push,pull,leg}.jpg** (note `leg`, singular; runtime URL via `import.meta.env.BASE_URL`). Jon's source PNGs were 1536×338 / ~660 KB with transparent rounded corners; optimized with **sharp** → flatten alpha onto `#0a0a0c` (kills the transparency "checkerboard", blends into the dark page), resize to 1024w, mozjpeg q84 → ~38 KB each (~17×). theme.js `dayImage(day)` maps day→file; swap the files to restyle, no code change.
