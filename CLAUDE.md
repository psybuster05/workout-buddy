...
- `startRest` must stay **fully synchronous** — iOS requires the WebAudio context unlock inside the original user-gesture call stack. All async work lives in the fire path, not the start path.
