...
3. **Exercise** — "This Set" card: a context line (`Set N` · `Last: …` · a live **overload cue** — accent "▲ ahead" / dim "= matched" vs last time), weight/reps via a scrollable **number ribbon** (see below), a **Failure | Warm-up** segmented toggle (warm-up sets are optional per-set flags, excluded from PRs), and "Finish set" → logs, auto-starts the per-exercise rest timer, and starts the workout clock on the first set of the day. Notes card (see below), History card (every logged set today carries its own **✕**, two-tap like History's row deletes — not just the most recent one), then YouTube embed (iframe) + written form cues. Back → Day.
   ...
   - **Notes** — a sticky per-exercise note, **read-only until you tap the pencil** (which becomes a ✓ while editing); an ✕ beside it clears (two-tap, armed → "Clear?", like History's deletes) and only shows when there's a note. The **PR line lives inside this card** — the best working set shown as real weight×reps + top weight ("PR · best 45 lbs × 12 · top 50 lbs"), or a dimmed "PR · None yet" (warm-up sets don't count; see `personalRecord` in format.js). The note's **placeholder is the programmed target** ("Sets: 3–4 · Reps: 8–12" / "Goal: 20–40 min"), which is the only remaining consumer of `target` in exercises.json.
...
5. **Account** — person icon in the header (button order: rest → sync → account; account shows whenever Supabase is configured). Imperial/Metric weight-unit toggle, then a **Bodyweight** card (shown in both signed-in and offline modes): a Ribbon weigh-in entry prefilled to your last entry ("Log today"/"Update today", idempotent per day) + a fold-up **weigh-in-history** accordion (latest on the closed row; each entry shows Δ-since-last with a two-tap delete). Signed in: "Signed in as" email card, change account email, sign out. Offline mode: explains device-only storage + "Log in to back up". The footer (© + build date) renders on History and Account only; the sync-status line renders at the bottom of **every** screen when logged in (one `.app-bottom` wrapper owns the sticky-bottom margin).
...
// localStorage store (key: "workout-tracker:v1")
{
  ...
  "bodyweight": {
    "2026-07-18": { "lbs": 170, "at": 1784388490839 }
  }
}
```
...
**`bodyweight`** — weigh-in log: maps **date** → `{ lbs, at }` (canonical lbs, one per calendar day; re-logging a day overwrites). Surfaced on the Account screen (Ribbon entry prefilled to the last weigh-in + a fold-up history accordion). Merges LWW via `lww()`; deleting stores `{ lbs: null, at }` as a **tombstone** (like a cleared note) so a stale cloud copy can't resurrect it. Backfilled to `{}`.
