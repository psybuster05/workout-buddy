...
- Day-button photos: **public/days/{push,pull,leg}.jpg** (note `leg`, singular; runtime URL via `import.meta.env.BASE_URL`). Jon's source PNGs were 1536×338 / ~660 KB with a **checkerboard baked into the pixels** (AI-gen faux-transparency, NOT real alpha — so flattening alpha did nothing) around a rounded banner with uneven margins (~35px sides, ~10px top/bottom). Fixed with **sharp**: brightness-scan each edge to the first dark row/col → find the banner bbox → rounded-rect mask (radius 36) → flatten outside onto `#0a0a0c` → resize 1024w, mozjpeg q84 → ~35 KB each (~18×). theme.js `dayImage(day)` maps day→file; swap the files to restyle, no code change.
