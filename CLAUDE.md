...
### 2026-07-16 — Exercise screen: This Set moved above Notes; delete any set, not just the last
Two small Jon-requested tweaks to the session zone.

- **Card order**: This Set now comes first, Notes second, History third — was Notes→This Set→History. Pure JSX reorder in Exercise.jsx; no state or storage change.
- **`storage.deleteLastSet(exerciseId)` → `deleteSetAt(exerciseId, index)`.** The old function only ever popped the final set; the new one splices any index (out-of-range is a no-op) and still drops the session entirely if that empties it, so no zero-set session is ever left behind. Old callers fully subsumed — `deleteSetAt(id, len-1)` is byte-for-byte the old behavior — so this was a replace, not an addition.
- **The single "Delete last set" button is gone.** Every set in the History card's drawer now gets its own **✕** (plus one on the collapsed summary row, so undoing the most recent set still doesn't require opening the drawer). Reused `.history-delete` — the exact two-tap arm→"Delete?"→3s-auto-disarm component History's own row deletes use — rather than inventing a second delete idiom. This is a deliberate reversal of the 2026-07-07 call ("a low-stakes undo, deliberately not the two-tap confirm") — that reasoning fit one big labelled button; a column of small ✕s next to already-logged sets is much easier to mis-tap, so the confirm step earns its keep here.
- Still only rendered when `historyIsToday` — a historical (non-today) session shows zero ✕s, same as before; those deletions stay on the History screen.
- **Verified in preview:** deleting the *middle* of three sets removes exactly that one and the rest renumber (the capability the old button never had); first tap arms without touching storage; deleting the last remaining set drops the session and the History card disappears; a historical session renders with no delete buttons at all.

### 2026-07-16 — Ribbon extended to Cardio; typed-stepper retired
Cardio's minutes/miles fields were the last place the old +/− stepper lived — every previous ribbon entry explicitly carved cardio out. Jon asked for the ribbon there too.

- **Minutes ribbon** (`min=0, max=180, step=1`) sits where the typed-minutes input was; **distance ribbon** (`min=0, max=30, step=0.1, decimals=1`) replaces the shared `weightRow` miles control. Same components, same props shape as the weighted/reps-only/time ribbons — cardio isn't a special case in Exercise.jsx anymore, just different bounds/unit.
- **`weightRow` and `adjustWeight` deleted.** Once the weighted-mode ribbon took over lifting weight, `weightRow` only had one caller left (cardio's miles field); replacing that caller made the whole variable and its `±delta` helper dead code.
- **All `.counter-*` CSS deleted** (`counter-row/button/value/value-wrap/sub`, the spin-button hider, the hover rule) — nothing in the app renders that markup anymore. The ribbon is now the only numeric input control in the app.
- **Verified in preview:** both cardio ribbons render with the day's accent, rapid chevron clicks accumulate (25 min / 2.1 mi, not dropped), Finish Set logs exactly `{reps: 25, weight: 2.1}` → "25 min · 2.1 mi", the **miles-optional path still works** (0 mi logs as `weight: 0` and displays as just "25 min", no stray "· 0 mi"), and each cardio exercise's own `restSeconds` still governs the auto-rest-timer (0 for Walk/Run, 60s for Jump Rope) — untouched, since that logic lives in `finishSet`, not the ribbon.
...
- **New `src/components/Ribbon.jsx`.** A horizontal scroll-snap tape marked in `step` increments (weight: `weightStep(unit)`; reps/seconds: 1), a fixed center caret over the selected tick, flanking `‹ ›` chevrons for a precise one-step nudge, and a tappable readout above that switches to a real `<input type="number">` for typing an exact value (flicking to 85 lbs one tick at a time is painful). Scope: weight (weighted mode) and reps/seconds (weighted, reps-only, time) in Exercise.jsx. **Cardio's minutes/miles fields keep their old typed-stepper** — not a weight/reps control.
- **Mouse gets first-class support**, not just an afterthought: click-and-drag the tape (pointer events, `pointerType !== 'touch'` so it doesn't fight native touch scrolling) and mouse-wheel scrolls it horizontally (a non-passive native `wheel` listener converts vertical deltaY, since React's synthetic `onWheel` can't `preventDefault`; a trackpad's horizontal deltaX is left alone). Touch/trackpad flicking is native `overflow-x: auto` + `scroll-snap-type`, unchanged.
- **Two bugs caught during verification, not after — both hidden by the tests-pass/build-green gate:**
  1. `fmt()` trimmed trailing zeros to shorten decimals (`45.00`→`"45"`), but the same regex also stripped a bare integer's trailing zero — **10 reps would have rendered as "1"**, 50→5, 100→1. Fixed to only trim the fractional part.
  2. The initial-position effect (scrolling the tape to match an external `value` change, e.g. a prefill) fired the scroll handler's `onChange` too — so mounting on an off-grid kg prefill (45 lbs = 20.4 kg, off the 1.25 kg step grid) **silently snapped it to 20 kg**, a value the user never chose. Fixed with a `suppress` ref so a programmatic reposition can't feed back into `onChange`; a real user scroll still does.
  - Also: the flanking chevrons initially dropped rapid clicks (3 fast taps only advanced one step) — the exact stale-closure bug the 2026-07-06 rep counter fix addressed, reintroduced by closing over the `value` prop. Fixed with an accumulating `valueRef`, verified 3 clicks → +3 steps.
- **Look pass** (same day): tick stride tightened 52px→36px (`ITEM`, single-sourced to CSS via an inline `--item` var so JS scroll-math and CSS layout can't drift), label frequency raised (weight every 5 units, reps every 2, seconds every 10), chevrons added either side of the tape.
- **Verified in preview**, including the two bugs above pre/post-fix: render + tick positioning from a prefill, scroll-driven value change with momentum preserved, tap-to-type (incl. `72.5`), Finish Set logging exactly the ribbon-shown values (`{reps: 8, weight: 72.5}`), re-prefill/reposition after logging, the kg unit path (labels, no-mutate-on-mount), mouse drag (readout tracked live, snapped clean on release), and mouse wheel (deltaY→horizontal). Untested from here: real touch/trackpad feel on a phone.

### 2026-07-16 — Finisher rows adopt the main list's styling
The Cardio/Core finisher rows still used the pre-redesign `.cardio-row` look (plain full-width button, left accent stripe) while the main list had the 2026-07-12 sporty treatment. `FinisherCard` now renders the same anatomy as the main list — outlined index numeral · `FitText` uppercase-italic name · accent chevron (✓ when done) — via the **shared `.exercise-button` / `.exercise-list` classes**. The finisher's borrowed `--accent` colours the numeral and chevron (amber Cardio, violet Core). Only a `.finisher-list` padding modifier is finisher-specific (the rows sit inside the `<details>` shell). All the `.cardio-list` / `.cardio-row` CSS is deleted (done-✓, `.is-off`, `:active`, hover all now come from `.exercise-button` for free). Verified: numeral/chevron accent, done ✓ + dimmed numeral, and edit-mode struck-through all render on finisher rows.
